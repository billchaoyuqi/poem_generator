...
# ------------------------------
def sample_mix(ori_keys, ori_dec_inps, ori_labels, clabels1, clabels2, clabels3, clabels4, clabels5,
    ...
    for sid in sample_ids:
        ...
        label1, label2, label3, label4, label5 = \
            ori_labels[sid, 0].item(), ori_labels[sid, 1].item(),\
            ori_labels[sid, 2].item(), ori_labels[sid, 3].item(),\
            ori_labels[sid, 4].item()
        flabel1, flabel2, flabel3, flabel4, flabel5 = \
            clabels1[sid].item(), clabels2[sid].item(),\
            clabels3[sid].item(), clabels4[sid].item(),\
            clabels5[sid].item()
        ...
        label_str3 = "factor3 label: %d" % (label3)
        if label3 == -1:
            label_str3 += ", inferred: %d" % (flabel3)

        label_str4 = "factor4 label: %d" % (label4)
        if label4 == -1:
            label_str4 += ", inferred: %d" % (flabel4)

        label_str5 = "factor5 label: %d" % (label5)
        if label5 == -1:
            label_str5 += ", inferred: %d" % (flabel5)
...
def factor_distance(mixpoet, ori_keys, n_class1, n_class2, n_class3, n_class4, n_class5, device):
    N = 64
    K = 8
    M = n_class1 * n_class2 * n_class3 * n_class4 * n_class5
    # print (M)
    ...
    with torch.no_grad():
        for l1 in range(0, n_class1):
            for l2 in range(0, n_class2):
                for l3 in range(0, n_class3):
                    for l4 in range(0, n_class4):
                        for l5 in range(0, n_class5):
                            for k in range(0, K):
                                labels1 = torch.tensor([l1] * N, dtype=torch.long, device=device)
                                labels2 = torch.tensor([l2] * N, dtype=torch.long, device=device)
                                labels3 = torch.tensor([l3] * N, dtype=torch.long, device=device)
                                labels4 = torch.tensor([l4] * N, dtype=torch.long, device=device)
                                labels5 = torch.tensor([l5] * N, dtype=torch.long, device=device)
                                points = mixpoet.compute_prior(keys, labels1, labels2, labels3, labels4, labels5)
