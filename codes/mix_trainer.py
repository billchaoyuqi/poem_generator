...
class MixTrainer(object):
    ...
    def run_validation(self, epoch, mixpoet, tool, lr):
        ...
        for step in range(0, tool.valid_batch_num):
            ...
            rec_loss, cl_loss_w, cl_loss_xw, entro_loss, _, _, _, _, _, _ = \
    ...
    def run_adv_step(self, mixpoet, optimizer, keys, poems,
        ...
        z_prior, z_post, cb_label1, cb_label2, cb_label3, cb_label4, cb_label5 \
        ...
        logits_real = mixpoet.layers['discriminator'](z_post + eps1, cb_label1, cb_label2, cb_label3, cb_label4, cb_label5)
        logits_fake = mixpoet.layers['discriminator'](z_prior + eps2, cb_label1, cb_label2, cb_label3, cb_label4, cb_label5)
    ...
    def run_rec_step(self, mixpoet, optimizer, keys, poems, dec_inps,
        ...
        all_outs, combined_label1, combined_label2, combined_label3, combined_label4, combined_label5, \
        ...
        cl_loss_w1, cl_loss_xw1, entro_loss_xw1 = self.losswrapper.cl_loss(logits_cl_w1,
                                                                           logits_cl_xw1, combined_label1,
                                                                           label_mask[:, 0])
        cl_loss_w2, cl_loss_xw2, entro_loss_xw2 = self.losswrapper.cl_loss(logits_cl_w2,
                                                                           logits_cl_xw2, combined_label2,
                                                                           label_mask[:, 1])
        cl_loss_w3, cl_loss_xw3, entro_loss_xw3 = self.losswrapper.cl_loss(logits_cl_w3,
                                                                           logits_cl_xw3, combined_label3,
                                                                           label_mask[:, 2])
        cl_loss_w4, cl_loss_xw4, entro_loss_xw4 = self.losswrapper.cl_loss(logits_cl_w4,
                                                                           logits_cl_xw4, combined_label4,
                                                                           label_mask[:, 3])
        cl_loss_w5, cl_loss_xw5, entro_loss_xw5 = self.losswrapper.cl_loss(logits_cl_w5,
                                                                           logits_cl_xw5, combined_label5,
                                                                           label_mask[:, 4])

        cl_loss_w = cl_loss_w1 + cl_loss_w2 + cl_loss_w3 + cl_loss_w4 + cl_loss_w5
        cl_loss_xw = cl_loss_xw1 + cl_loss_xw2 + cl_loss_xw3 + cl_loss_xw4 + cl_loss_xw5
        entro_loss_xw = entro_loss_xw1 + entro_loss_xw2 + entro_loss_xw3 + entro_loss_xw4 + entro_loss_xw5
        ...
        return rec_loss.item(), cl_loss_w.item(), cl_loss_xw.item(), \
               entro_loss_xw.item(), all_outs, combined_label1, combined_label2, combined_label3, combined_label4, combined_label5
    ...
    def gen_from_prior(self, mixpoet, keys, poems, dec_inps,
        ...
        with torch.no_grad():
            all_outs, cb_label1, cb_label2, cb_label3, cb_label4, cb_label5, logits_cl_xw1, logits_cl_xw2, logits_cl_xw3, logits_cl_xw4, logits_cl_xw5, logits_cl_w1, logits_cl_w2, logits_cl_w3, logits_cl_w4, logits_cl_w5 = \
    ...
    def run_train(self, mixpoet, tool, optimizerRec, optimizerDis,
        ...
        for step in range(0, tool.train_batch_num):
            ...
            rec_loss, cl_loss_w, cl_loss_xw, entro_loss, outs_post, clabels1, clabels2, clabels3, clabels4, clabels5 = \
            ...
            if (step % 40 == 0) and (logger.total_steps > self.hps.rec_warm_steps):
                ...
                fadist = utils.factor_distance(mixpoet, batch_keys,
                                               self.hps.n_class1, self.hps.n_class2, self.hps.n_class3, self.hps.n_class4, self.hps.n_class5, device)
            ...
            if step % self.hps.log_steps == 0:
                ...
                utils.sample_mix(batch_keys, batch_dec_inps, batch_labels,
                                 clabels1, clabels2, clabels3, clabels4, clabels5, outs_post, outs_prior, self.hps.sample_num, tool)
                logger.print_log()
                logger.draw_curves()
                logger.set_start_time()
