...
class Discriminator(nn.Module):
    def __init__(self, n_class1, n_class2, n_class3, n_class4, n_class5, factor_emb_size,
        ...
        self.inp2feature = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            SN(nn.Linear(latent_size, factor_emb_size * 5)),
            nn.LeakyReLU(0.2),
            SN(nn.Linear(factor_emb_size * 5, factor_emb_size * 5)),
            nn.LeakyReLU(0.2))
        self.feature2logits = SN(nn.Linear(factor_emb_size * 5, 1))
        ...
        self.dis_fembed3 = SN(nn.Embedding(n_class3, factor_emb_size))
        self.dis_fembed4 = SN(nn.Embedding(n_class4, factor_emb_size))
        self.dis_fembed5 = SN(nn.Embedding(n_class5, factor_emb_size))

    def forward(self, x, labels1, labels2, labels3, labels4, labels5):
        # x, latent variable: (B, latent_size)

        femb1 = self.dis_fembed1(labels1)
        femb2 = self.dis_fembed2(labels2)
        femb3 = self.dis_fembed3(labels3)
        femb4 = self.dis_fembed4(labels4)
        femb5 = self.dis_fembed5(labels5)
        factor_emb = torch.cat([femb1, femb2, femb3, femb4, femb5], dim=1)  # (B, factor_emb_size*2)
...
class PriorGenerator(nn.Module):
    def __init__(self, inp_size, latent_size, n_class1, n_class2, n_class3, n_class4, n_class5, factor_emb_size):
        ...
        self.factor_embed3 = nn.Embedding(n_class3, factor_emb_size)
        self.factor_embed4 = nn.Embedding(n_class4, factor_emb_size)
        self.factor_embed5 = nn.Embedding(n_class5, factor_emb_size)
        ...
        self.mlp3 = nn.Sequential(
            CBNLayer(inp_size + factor_emb_size, latent_size, n_class3),
            CBNLayer(latent_size, latent_size, n_class3))
        self.bn3 = nn.Sequential(
            nn.Linear(latent_size, self.slatent_size),
            nn.BatchNorm1d(self.slatent_size))

        self.mlp4 = nn.Sequential(
            CBNLayer(inp_size + factor_emb_size, latent_size, n_class4),
            CBNLayer(latent_size, latent_size, n_class4))
        self.bn4 = nn.Sequential(
            nn.Linear(latent_size, self.slatent_size),
            nn.BatchNorm1d(self.slatent_size))

        self.mlp5 = nn.Sequential(
            CBNLayer(inp_size + factor_emb_size, latent_size, n_class5),
            CBNLayer(latent_size, latent_size, n_class5))
        self.bn5 = nn.Sequential(
            nn.Linear(latent_size, self.slatent_size),
            nn.BatchNorm1d(self.slatent_size))

        # 添加最终的调整尺寸的线性层
        self.final_adjustment = nn.Linear(5 * self.slatent_size, latent_size)

    def forward(self, key_state, labels1, labels2, labels3, labels4, labels5):
        factor1 = self.factor_embed1(labels1)
        factor2 = self.factor_embed2(labels2)
        factor3 = self.factor_embed1(labels3)
        factor4 = self.factor_embed2(labels4)
        factor5 = self.factor_embed1(labels5)
        ...
        eps3 = torch.randn((batch_size, self.slatent_size),
                           dtype=torch.float, device=device)
        cond3 = torch.cat([eps3, key_state, factor3], dim=1)
        prior3 = self.mlp3((cond3, labels3))[0]
        prior3 = self.bn3(prior3)

        eps4 = torch.randn((batch_size, self.slatent_size),
                           dtype=torch.float, device=device)
        cond4 = torch.cat([eps4, key_state, factor4], dim=1)
        prior4 = self.mlp4((cond4, labels4))[0]
        prior4 = self.bn4(prior4)

        eps5 = torch.randn((batch_size, self.slatent_size),
                           dtype=torch.float, device=device)
        cond5 = torch.cat([eps5, key_state, factor5], dim=1)
        prior5 = self.mlp5((cond5, labels5))[0]
        prior5 = self.bn5(prior5)

        z_prior = torch.cat([prior1, prior2, prior3, prior4, prior5], dim=1)

        # 使用 final_adjustment 层调整输出尺寸
        z_prior = self.final_adjustment(z_prior)
        return z_prior
...
class PosterioriGenerator(nn.Module):
    def __init__(self, inp_size, latent_size, n_class1, n_class2, n_class3, n_class4, n_class5, factor_emb_size):
        ...
        self.post_embed3 = nn.Embedding(n_class3, factor_emb_size)
        self.post_embed4 = nn.Embedding(n_class4, factor_emb_size)
        self.post_embed5 = nn.Embedding(n_class5, factor_emb_size)

        self.mlp = MLP(
            inp_size + factor_emb_size * 5 + latent_size,
            layer_sizes=[512, latent_size, latent_size],
            activs=['leakyrelu', 'leakyrelu', None], no_drop=True)

    def forward(self, sen_state, key_state, labels1, labels2, labels3, labels4, labels5):
        factor1 = self.post_embed1(labels1)
        factor2 = self.post_embed2(labels2)
        factor3 = self.post_embed3(labels3)
        factor4 = self.post_embed4(labels4)
        factor5 = self.post_embed5(labels5)

        batch_size = key_state.size(0)
        eps = torch.randn((batch_size, self.latent_size),
                          dtype=torch.float, device=device)

        cond = torch.cat([sen_state, key_state, factor1, factor2, factor3, factor4, factor5, eps], dim=1)  #
