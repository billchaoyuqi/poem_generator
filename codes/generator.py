...
class Generator(object):
    ...
    def generate_one(self, keyword, length, factor_label1, factor_label2, factor_label3, factor_label4, factor_label5,
        ...
        if factor_label3 == -1:
            factor_label3 = self.get_inferred_factor(key_state, 3)
            if verbose >= 1:
                print("inferred label3: %d" % (factor_label3[0].item()))
        else:
            factor_label3 = torch.tensor([factor_label3],
                                         dtype=torch.long, device=device)

        if factor_label4 == -1:
            factor_label4 = self.get_inferred_factor(key_state, 4)
            if verbose >= 1:
                print("inferred label4: %d" % (factor_label4[0].item()))
        else:
            factor_label4 = torch.tensor([factor_label4],
                                         dtype=torch.long, device=device)

        if factor_label5 == -1:
            factor_label5 = self.get_inferred_factor(key_state, 5)
            if verbose >= 1:
                print("inferred label5: %d" % (factor_label5[0].item()))
        else:
            factor_label5 = torch.tensor([factor_label5],
                                         dtype=torch.long, device=device)

        # get decoder initial state

        dec_init_state = self.get_dec_init_state(
            key_state, factor_label1, factor_label2, factor_label3, factor_label4, factor_label5, length)
        ...
        for step in range(0, self.hps.sens_num):
            ...
            if manu:
                ...
                which, ok = QInputDialog.getInt(None, "选择诗句", f"请选择一句诗句:\n{poem_options}", 0, 0, len(lines) - 1)
    ...
    def get_inferred_factor(self, key_state, factor_id):
        assert factor_id == 1 or factor_id == 2 or factor_id == 3 or factor_id == 4 or factor_id == 5
        return self.model.compute_inferred_label(key_state, factor_id)

    def get_dec_init_state(self, key_state, label1, label2, label3, label4, label5, length):
        state = self.model.compute_dec_init_state(key_state, label1, label2, label3, label4, label5)
