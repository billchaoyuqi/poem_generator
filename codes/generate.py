...
def parse_args():
    ...
    parser.add_argument("-b", "--bsize", type=int, default=20,
                        help="beam size, 20 by default.")
...
def generate_manu(args):
    ...
    while True:
        ...
        label3 = int(input("specify the number label\n\
                    0: no, 1: yes, -1: not specified>"))
        label4 = int(input("specify the top_needle label\n\
                    0: no, 1: yes, -1: not specified>"))
        label5 = int(input("specify the tiangan_dizhi label\n\
                    0: no, 1: yes, -1: not specified>"))

        lines, info = generator.generate_one(keyword, length, label1, label2, label3, label4, label5,
