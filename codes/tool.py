...
class Tool(object):
    ...
    def build_data(self, train_data_path, valid_data_path, batch_size, mode):
        ...
        mode:
            cln: pre-train the classifier for factor n
        ...
        '''
        assert mode in ['cl1', 'cl2', 'cl3', 'cl4', 'cl5', 'dae', 'mixpoet_pre', 'mixpoet_tune']
    ...
    def __build_data_core(self, data, batch_size, mode, data_limit=None):
        ...
        if mode == 'cl1' or mode == 'cl2' or mode == 'cl3' or mode == 'cl4' or mode == 'cl5':
    ...
    def build_mixpoet_batches(self, ori_data, batch_size, mode):
        ...
        if mode == 'mixpoet_pre':
            ...
            for instance in ori_data:
                ...
                label4 = instance[5]
                label5 = instance[6]
                if label1 == -1 and label2 == -1 and label3 == -1 and label4 == -1 and label5 == -1:
                    continue
        ...
        for bi in range(0, batch_num):
            ...
            # build labels
            labels = [(instance[2], instance[3], instance[4], instance[5], instance[6]) for instance in instances]
            label_mask = [(float(pair[0] != -1), float(pair[1] != -1),
                           float(pair[2] != -1), float(pair[3] != -1),
                           float(pair[4] != -1)) for pair in labels]
