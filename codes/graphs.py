...
class MixPoetAUS(nn.Module):
    def __init__(self, hps):
        ...
        self.n_class3 = hps.n_class3
        self.n_class4 = hps.n_class4
        self.n_class5 = hps.n_class5
        ...
        self.layers['cl_xw2'] = MLP(self.hidden_size * 2 + self.emb_size * 2,
                                    ...
                                    drop_ratio=hps.drop_ratio)
        self.layers['cl_xw3'] = MLP(self.hidden_size * 2 + self.emb_size * 2,
                                    layer_sizes=[self.hidden_size, 128, self.n_class3], activs=['relu', 'relu', None],
                                    drop_ratio=hps.drop_ratio)
        self.layers['cl_xw4'] = MLP(self.hidden_size * 2 + self.emb_size * 2,
                                    layer_sizes=[self.hidden_size, 128, self.n_class4], activs=['relu', 'relu', None],
                                    drop_ratio=hps.drop_ratio)
        self.layers['cl_xw5'] = MLP(self.hidden_size * 2 + self.emb_size * 2,
                                    layer_sizes=[self.hidden_size, 128, self.n_class5], activs=['relu', 'relu', None],
                                    drop_ratio=hps.drop_ratio)
        ...
        self.layers['cl_w2'] = MLP(self.emb_size * 2,
                                   ...
                                   drop_ratio=hps.drop_ratio)
        self.layers['cl_w3'] = MLP(self.emb_size * 2,
                                   layer_sizes=[self.emb_size, 64, self.n_class3], activs=['relu', 'relu', None],
                                   drop_ratio=hps.drop_ratio)
        self.layers['cl_w4'] = MLP(self.emb_size * 2,
                                   layer_sizes=[self.emb_size, 64, self.n_class4], activs=['relu', 'relu', None],
                                   drop_ratio=hps.drop_ratio)
        self.layers['cl_w5'] = MLP(self.emb_size * 2,
                                   layer_sizes=[self.emb_size, 64, self.n_class5], activs=['relu', 'relu', None],
                                   drop_ratio=hps.drop_ratio)
        ...
        self.layers['factor_embed3'] = nn.Embedding(self.n_class3, self.factor_emb_size)
        self.layers['factor_embed4'] = nn.Embedding(self.n_class4, self.factor_emb_size)
        self.layers['factor_embed5'] = nn.Embedding(self.n_class5, self.factor_emb_size)

        # posteriori and prior
        self.layers['prior'] = PriorGenerator(
            self.emb_size * 2 + int(self.latent_size // 2),
            self.latent_size,
            self.n_class1, self.n_class2, self.n_class3, self.n_class4, self.n_class5,
            self.factor_emb_size)

        self.layers['posteriori'] = PosterioriGenerator(
            self.hidden_size * 2 + self.emb_size * 2,
            self.latent_size,
            self.n_class1, self.n_class2, self.n_class3, self.n_class4, self.n_class5,
            self.factor_emb_size)

        # for adversarial training
        self.layers['discriminator'] = Discriminator(self.n_class1, self.n_class2, self.n_class3, self.n_class4, self.n_class5,
        ...
        #   classical quatrains.
        self.layers['dec_init'] = MLP(self.latent_size + self.emb_size * 2 + self.factor_emb_size * 5,
    ...
    def get_prior_and_posterior(self, key_inps, vae_inps, factor_labels,
        ...
        factor_emb3, logits_cl_xw3, combined_label3 = self.get_factor_emb(condition, 3,
                                                                          factor_labels[:, 2], factor_mask[:, 2])
        factor_emb4, logits_cl_xw4, combined_label4 = self.get_factor_emb(condition, 4,
                                                                          factor_labels[:, 3], factor_mask[:, 3])
        factor_emb5, logits_cl_xw5, combined_label5 = self.get_factor_emb(condition, 5,
                                                                          factor_labels[:, 4], factor_mask[:, 4])
        factors = torch.cat([factor_emb1, factor_emb2, factor_emb3, factor_emb4, factor_emb5], dim=-1)
        ...
        z_post = self.layers['posteriori'](sen_state, key_state,
                                           combined_label1, combined_label2,
                                           combined_label3, combined_label4,
                                           combined_label5)
        z_prior = self.layers['prior'](key_state,
                                       combined_label1, combined_label2,
                                       combined_label3, combined_label4,
                                       combined_label5)
        ...
        if ret_others:
            return z_prior, z_post, key_state, factors, \
                   logits_cl_xw1, logits_cl_xw2, logits_cl_xw3, logits_cl_xw4, logits_cl_xw5,\
                   combined_label1, combined_label2, combined_label3, combined_label4, combined_label5
        else:
    ...
    def forward(self, key_inps, vae_inps, dec_inps, factor_labels, factor_mask, lengths,
        ...
        z_prior, z_post, key_state, factors,\
        logits_cl_xw1, logits_cl_xw2, logits_cl_xw3, logits_cl_xw4, logits_cl_xw5,\
        cb_label1, cb_label2, cb_label3, cb_label4, cb_label5\
        ...
        logits_cl_w3 = self.layers['cl_w3'](key_state)
        logits_cl_w4 = self.layers['cl_w4'](key_state)
        logits_cl_w5 = self.layers['cl_w5'](key_state)

        return all_gen_outs, cb_label1, cb_label2, cb_label3, cb_label4, cb_label5,\
               logits_cl_xw1, logits_cl_xw2, logits_cl_xw3, logits_cl_xw4, logits_cl_xw5, \
               logits_cl_w1, logits_cl_w2, logits_cl_w3, logits_cl_w4, logits_cl_w5
    ...
    def rec_parameters(self):
        ...
        names = ['embed', 'encoder', 'decoder', 'word_encoder',
                 'cl_xw1', 'cl_xw2', 'cl_xw3', 'cl_xw4', 'cl_xw5',
                 'cl_w1', 'cl_w2', 'cl_w3', 'cl_w4', 'cl_w5',
                 'factor_embed1', 'factor_embed2', 'factor_embed3', 'factor_embed4', 'factor_embed5',
                 'posteriori', 'out_proj', 'dec_init', 'context', 'map_x']
    ...
    def compute_dec_init_state(self, key_state, labels1, labels2, labels3, labels4, labels5):
        z_prior = self.layers['prior'](key_state, labels1, labels2, labels3, labels4, labels5)

        factor_emb1 = self.layers['factor_embed1'](labels1)
        factor_emb2 = self.layers['factor_embed2'](labels2)
        factor_emb3 = self.layers['factor_embed3'](labels3)
        factor_emb4 = self.layers['factor_embed4'](labels4)
        factor_emb5 = self.layers['factor_embed5'](labels5)

        dec_init_state = self.layers['dec_init'](
            torch.cat([z_prior, key_state, factor_emb1, factor_emb2, factor_emb3, factor_emb4, factor_emb5],
                      dim=-1))  # (B, H-2)

        return dec_init_state

    def compute_prior(self, keys, labels1, labels2, labels3, labels4, labels5):
        _, key_state0 = self.computer_enc(keys, self.layers['word_encoder'])
        key_state = torch.cat([key_state0[0, :, :], key_state0[1, :, :]], dim=-1)  # [B, 2*H]
